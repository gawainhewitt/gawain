...
function preload() {
  ...
  seqOff = loadImage(`/connectivity/images/chair.png`);
  seqStep1 = loadImage(`/connectivity/images/chairGreen.png`);
  seqStep2 = loadImage(`/connectivity/images/chairBlue.png`);
  grass = carpet;
  looper = loadImage(`/connectivity/images/wigmorestage2.png`);
  for(let i = 0; i < numberOfloopers; i++){
    performerOff[i] = loadImage(`/connectivity/images/performerOff${i+1}.png`);
    performerOn[i] = loadImage(`/connectivity/images/performerOn${i+1}.png`);
  }
  ...
  buffers = new Tone.ToneAudioBuffers({
    ...
    baseUrl: "/connectivity/sounds/"
  });

  for(let i = 0; i < seqRows; i++){
    seqBuffers[i] = new Tone.ToneAudioBuffer(`/connectivity/sounds/${stepName[i]}.flac`)
  }
